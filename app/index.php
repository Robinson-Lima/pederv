        ...
        if(!$fromMe){
          ...
          if($botActive==='1'){
            ...
            if($isSaas){
              ...
              foreach(master_db()->query("SELECT * FROM saas_bot_replies WHERE ativo=1")->fetchAll() as $br)foreach(explode('|',mb_strtolower($br['gatilho'])) as $kw)if(trim($kw)!==''&&mb_strpos($low,trim($kw))!==false){$reply=$br['resposta'];break 2;}
              if($reply!==''){
                ...
                if($saasUrl&&$saasKey&&$saasInst){
                  ...
                  curl_setopt_array($ch,[CURLOPT_CUSTOMREQUEST=>'POST',CURLOPT_RETURNTRANSFER=>true,CURLOPT_CONNECTTIMEOUT=>5,CURLOPT_TIMEOUT=>15,CURLOPT_HTTPHEADER=>['apikey: '.$saasKey,'Content-Type: application/json'],CURLOPT_POSTFIELDS=>json_encode(['number'=>$waNum,'text'=>$reply,'delay'=>300,'linkPreview'=>true],JSON_UNESCAPED_UNICODE)]);
                  $raw=curl_exec($ch);$code=(int)curl_getinfo($ch,CURLINFO_HTTP_CODE);curl_close($ch);
...
case 'saas_bot_save':
  ...
  if($tab==='config'){
    ...
    $gatilhos=$_POST['gatilho']??[];
    $respostas=$_POST['resposta']??[];
    $ativos=$_POST['ativo']??[];
    $ins=master_db()->prepare("INSERT INTO saas_bot_replies(gatilho,resposta,ativo) VALUES(?,?,?)");
    foreach($gatilhos as $i=>$g){
      if(trim($g)===''&&trim($respostas[$i]??'')===''){ continue; }
      $ins->execute([trim($g),trim($respostas[$i]??''),isset($ativos[$i])?1:0]);
    }
  }
